// Debug routes for testing (development only)
...
router.get('/canvas-test', (req: Request, res: Response) => {
  try {
    ...
    ctx.fillText('Canvas + Fonts OK', 200, 100);

    if (req.query.format === 'image') {
      res.set('Content-Type', 'image/png').send(canvas.toBuffer('image/png'));
    } else {
      res.json({ success: true, image: canvas.toDataURL('image/png') });
    }
  } catch (err) {
...
function testCard(isProfit: boolean): CardData {
  const [entry, current] = isProfit ? [0.0000024, 0.0000120] : [0.10, 0.05];
  const pnl = calculatePnL(entry, current);
  if ('error' in pnl) throw new Error(pnl.message);
  return {
    tokenSymbol: isProfit ? 'PEPE' : 'WOJAK', entryPrice: entry, currentPrice: current, pnl,
...
const themeHandler = (theme: Theme) => (req: Request, res: Response) => {
  try {
    ...
    if (req.query.format === 'json') {
      res.json({ success: true, theme, image: `data:image/png;base64,${buf.toString('base64')}` });
    } else {
